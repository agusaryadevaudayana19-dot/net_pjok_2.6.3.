...
class DataStorageService {
  ...
  public permanentDeleteFromTrash(trashIdsOrUserIds: string[] | string, actorName: string = 'Administrator'): void {
    ...
    this.updateDatabase((prev) => {
      ...
      // Hapus seluruh relasi data siswa terkait agar bersih total
      const log: ActivityLog = {
  ...
  public importMuridBatch(newMuridList: User[], actorName: string = 'Administrator'): void {
    ...
    this.updateDatabase((prev) => {
      ...
      // Hapus dari bak sampah jika siswa yang diimport sebelumnya terhapus ke bak sampah
      ...
      for (const m of newMuridList) {
        ...
        // Cari apakah siswa ini sudah ada di database (berdasarkan NIS atau Nama + Kelas)
        let matchIdx = -1;
        if (cleanNis) {
        ...
        if (matchIdx >= 0) {
          // UPDATE siswa yang sudah ada (pertahankan ID lama agar rekap nilai & absensi tidak putus)
          const old = existingUsers[matchIdx];
          existingUsers[matchIdx] = {
            ...old,
            ...m,
            id: old.id,
            username: old.username || m.username || (cleanNis ? `siswa_${cleanNis}` : `siswa_${Date.now()}`),
          ...
          const cleanUsername = (m.username || (cleanNis ? `siswa_${cleanNis}` : '') || `siswa_${Date.now()}`).toLowerCase().trim();
          existingUsers.push({
            ...m,
            id: finalId,
