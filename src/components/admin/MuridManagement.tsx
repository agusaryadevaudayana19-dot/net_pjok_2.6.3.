...
export const MuridManagement: React.FC<MuridManagementProps> = ({ db }) => {
  ...
  const handleConfirmClearAll = () => {
    ...
    showToast(`Seluruh data murid (${count} siswa) telah dipindahkan ke Bak Sampah. Data murid kini kosong dan siap diisi melalui import CSV/Excel.`);
  };
  ...
  const handleSaveMurid = (e: React.FormEvent) => {
    ...
    if (editingMurid) {
      ...
      const newMurid: User = {
        ...
        name: formData.name!.trim(),
        username: (formData.username?.trim() || formData.nis?.trim() || `siswa_${Date.now()}`).toLowerCase(),
        role: 'MURID',
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <span className="text-[11px] text-slate-400">
                              {m.email || `${m.username}@siswa.sman1olahraga.sch.id`}
                            </span>
                          </div>
                        </div>
                      </td>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-md w-full p-6 space-y-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-mono mt-0.5">{detailMurid.email || `${detailMurid.username}@siswa.sch.id`}</p>
        ...
        <div className="fixed inset-0 z-60 flex items-center justify-center bg-slate-950/70 backdrop-blur-xs p-4 animate-in fade-in">
          <div className="bg-white rounded-3xl border border-rose-200 p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                Seluruh {muridList.length} data murid aktif akan dipindahkan ke <strong>Bak Sampah</strong>. Tabel murid akan menjadi kosong sehingga Anda dapat mengimpor data siswa baru secara bersih melalui file CSV/Excel. Data murid yang lama tetap dapat Anda kembalikan kapan saja dari menu Bak Sampah.
              </p>
            </div>
            <div className="flex items-center justify-end gap-2 pt-2">
              <button
    ...
    reader.onload = (event) => {
      try {
        ...
        for (let i = 1; i < lines.length; i++) {
          ...
          const name = rowObj['nama'] || rowObj['name'] || rowObj['nama siswa'] || rowObj['nama lengkap'] || '';
          if (name) {
            rows.push({
              name,
    ...
    try {
      const usersToInsert: User[] = parsedData.map((row, idx) => {
        ...
        const baseUsername = row.username
          ...
          : cleanNis
          ? `siswa_${cleanNis}`
          : cleanName.toLowerCase().replace(/[^a-z0-9]/g, '_').slice(0, 15) || `siswa_${idx}`;
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl max-w-xl w-full p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-black text-slate-900">
                Import Data Murid (Khusus Siswa)
              </h3>
              <p className="text-xs text-slate-500">
